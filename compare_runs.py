...
def compare_metrics(metrics1: Dict[str, Any], metrics2: Dict[str, Any]) -> list:
    ...
    metric_keys = ["best_val_metric", "best_val_final_equity_proxy", "final_equity", "n_trades"]
    ...
    for key in metric_keys:
        ...
        if v1 is not None and v2 is not None:
            ...
            comparisons.append([
                ...
                f"{diff:+.6f}",
                f"{diff_pct:+.2f}%" if isinstance(v1, (int, float)) and isinstance(v2, (int, float)) else "N/A",
            ])
        
        if v1 is not None and v2 is not None and "holdout" in key:
            comparisons.append([
                key,
                str(v1),
                str(v2),
                str(v2 - v1) if isinstance(v1, (int, float)) else "N/A",
                "",
            ])
