...
main_cli.py  ──────────────────────────────────────────────────────►  Excel + JSON
     │
     ├── data.py                 加载并清洗 A 股 OHLCV Excel 数据
     │
     ├── training.py             随机搜索（探索 / 精细化 / 池采样 三模式轮次）
     │       ├── feature_dpoint.py   构建特征矩阵 X 和标签 y
     │       ├── models.py         sklearn 模型（LogReg、SGD、XGB）+ PyTorch 模型
     │       ├── data.py           Walk-Forward 时序切分
     │       ├── evaluation.py     几何均值净值比率 + 交易次数惩罚项
     │       └── utils.py          best_so_far.json / best_pool.json 持久化
     │
     ├── evaluation.py           事件驱动回测（含 A 股约束）
     ├── reporting.py            Excel 工作簿 + JSON + HTML 报告
...
│
├── data.py                 Excel 加载器，含 OHLCV 数据验证 + Walk-Forward 切分
├── feature_dpoint.py       特征工程（全特征族 + 技术指标）
├── models.py               模型工厂（sklearn + PyTorch 统一）
│
├── training.py             合并模块：随机搜索 + 校准 + 解释器 + 持久化
├── evaluation.py           合并模块：回测引擎 + 风险指标 + 市场状态分析
├── reporting.py            Excel 工作簿 + JSON + HTML 报告
├── rolling_trainer.py      滚动再训练调度器（扩展/滚动窗口）
